	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert (appdata v, float3 normal : NORMAL)
			{
				...
				float c_offsetvert = pow(worldPos.x - _OffsetX, 2) + pow(worldPos.z - _OffsetZ, 2);
				...
				// XXX:TODO Try lerp
					float fractionRemaining = 1.0 - pow(impactDistance/_WaveTravelDistance, 5.0);
					_Amplitude *= fractionRemaining;
				} else {
					_Amplitude = 0.0;
				}
